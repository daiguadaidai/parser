...
LDFLAGS += -X "github.com/daiguadaidai/parser/mysql.TiDBReleaseVersion=$(shell git describe --tags --dirty)"
LDFLAGS += -X "github.com/daiguadaidai/tidb/util/printer.TiDBBuildTS=$(shell date -u '+%Y-%m-%d %I:%M:%S')"
LDFLAGS += -X "github.com/daiguadaidai/tidb/util/printer.TiDBGitHash=$(shell git rev-parse HEAD)"
LDFLAGS += -X "github.com/daiguadaidai/tidb/util/printer.TiDBGitBranch=$(shell git rev-parse --abbrev-ref HEAD)"
LDFLAGS += -X "github.com/daiguadaidai/tidb/util/printer.GoVersion=$(shell go version)"

TEST_LDFLAGS =  -X "github.com/daiguadaidai/tidb/config.checkBeforeDropLDFlag=1"
...
ifeq ("$(TRAVIS_COVERAGE)", "1")
	...
	$(OVERALLS) -project=github.com/daiguadaidai/tidb -covermode=count -ignore='.git,vendor,cmd,docs,LICENSES' -concurrency=1 || { $(GOFAIL_DISABLE); exit 1; }
else
...
checkdep:
	$(GO) list -f '{{ join .Imports "\n" }}' github.com/daiguadaidai/tidb/store/tikv | grep ^github.com/daiguadaidai/parser$$ || exit 0; exit 1
