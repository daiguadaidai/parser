...
import (
	...
	"github.com/daiguadaidai/tidb/ddl/testutil"
	ddlutil "github.com/daiguadaidai/tidb/ddl/util"
...
func (s *testFailDBSuite) TestHalfwayCancelOperations(c *C) {
	gofail.Enable("github.com/daiguadaidai/tidb/ddl/truncateTableErr", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/tidb/ddl/truncateTableErr")
	...
	gofail.Enable("github.com/daiguadaidai/tidb/ddl/renameTableErr", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/tidb/ddl/renameTableErr")
...
func (s *testFailDBSuite) TestInitializeOffsetAndState(c *C) {
	...
	gofail.Enable("github.com/daiguadaidai/tidb/ddl/uninitializedOffsetAndState", `return(true)`)
	tk.MustExec("ALTER TABLE t MODIFY COLUMN b int FIRST;")
	gofail.Disable("github.com/daiguadaidai/tidb/ddl/uninitializedOffsetAndState")
}

func (s *testFailDBSuite) TestUpdateHandleFailed(c *C) {
	gofail.Enable("github.com/daiguadaidai/tidb/ddl/errorUpdateReorgHandle", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/tidb/ddl/errorUpdateReorgHandle")
	tk := testkit.NewTestKit(c, s.store)
...
func (s *testFailDBSuite) TestAddIndexFailed(c *C) {
	gofail.Enable("github.com/daiguadaidai/tidb/ddl/mockAddIndexErr", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/tidb/ddl/mockAddIndexErr")
	tk := testkit.NewTestKit(c, s.store)
...
func (s *testFailDBSuite) TestGenGlobalIDFail(c *C) {
	defer gofail.Disable("github.com/daiguadaidai/tidb/ddl/mockGenGlobalIDFail")
	tk := testkit.NewTestKit(c, s.store)
	...
	for idx, test := range testcases {
		if test.mockErr {
			gofail.Enable("github.com/daiguadaidai/tidb/ddl/mockGenGlobalIDFail", `return(true)`)
			_, err := tk.Exec(test.sql)
			c.Assert(err, NotNil, Commentf("the %dth test case '%s' fail", idx, test.sql))
		} else {
			gofail.Enable("github.com/daiguadaidai/tidb/ddl/mockGenGlobalIDFail", `return(false)`)
...
func (s *testFailDBSuite) TestAddIndexWorkerNum(c *C) {
	...
	gofail.Enable("github.com/daiguadaidai/tidb/ddl/checkIndexWorkerNum", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/tidb/ddl/checkIndexWorkerNum")
