...
import (
	...
	"github.com/daiguadaidai/tidb/util/gcutil"
...
func (s *testSerialSuite) TestCancelAddIndexPanic(c *C) {
	gofail.Enable("github.com/daiguadaidai/tidb/ddl/errorMockPanic", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/tidb/ddl/errorMockPanic")
...
func (s *testSerialSuite) TestRestoreTableByJobIDFail(c *C) {
	...
	hook.OnJobRunBeforeExported = func(job *model.Job) {
		if job.Type == model.ActionRestoreTable {
			gofail.Enable("github.com/daiguadaidai/tidb/store/tikv/mockCommitError", `return(true)`)
			gofail.Enable("github.com/daiguadaidai/tidb/ddl/mockRestoreTableCommitErr", `return(true)`)
		}
	}
	origHook := s.dom.DDL().GetHook()
	defer s.dom.DDL().(ddl.DDLForTest).SetHook(origHook)
	...
	// do restore table.
	tk.MustExec(fmt.Sprintf("admin restore table by job %d", jobID))
	gofail.Disable("github.com/daiguadaidai/tidb/store/tikv/mockCommitError")
	gofail.Disable("github.com/daiguadaidai/tidb/ddl/mockRestoreTableCommitErr")
