...
func (s *testSessionSuite) TestFailStatementCommit(c *C) {
	...
	gofail.Enable("github.com/daiguadaidai/tidb/session/mockStmtCommitError", `return(true)`)
	_, err := tk.Exec("insert into t values (2),(3),(4),(5)")
	c.Assert(err, NotNil)

	gofail.Disable("github.com/daiguadaidai/tidb/session/mockStmtCommitError")
...
func (s *testSessionSuite) TestFailStatementCommitInRetry(c *C) {
	...
	gofail.Enable("github.com/daiguadaidai/tidb/session/mockCommitError8942", `return(true)`)
	gofail.Enable("github.com/daiguadaidai/tidb/session/mockStmtCommitError", `return(true)`)
	_, err := tk.Exec("commit")
	c.Assert(err, NotNil)
	gofail.Disable("github.com/daiguadaidai/tidb/session/mockCommitError8942")
	gofail.Disable("github.com/daiguadaidai/tidb/session/mockStmtCommitError")
...
func (s *testSessionSuite) TestGetTSFailDirtyStateInretry(c *C) {
	defer gofail.Disable("github.com/daiguadaidai/tidb/session/mockCommitError")
	defer gofail.Disable("github.com/daiguadaidai/tidb/session/mockGetTSErrorInRetry")

	tk := testkit.NewTestKitWithInit(c, s.store)
	tk.MustExec("create table t (id int)")

	gofail.Enable("github.com/daiguadaidai/tidb/session/mockCommitError", `return(true)`)
	gofail.Enable("github.com/daiguadaidai/tidb/session/mockGetTSErrorInRetry", `return(true)`)
	tk.MustExec("insert into t values (2)")
