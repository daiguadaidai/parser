...
import (
	...
	"github.com/daiguadaidai/tidb/store/mockoracle"
...
func (s *testGCWorkerSuite) TestDoGCForOneRegion(c *C) {
	...
	gofail.Enable("github.com/daiguadaidai/tidb/store/tikv/tikvStoreSendReqResult", `return("timeout")`)
	regionErr, err = taskWorker.doGCForRegion(bo, 20, loc.Region)
	c.Assert(regionErr, IsNil)
	c.Assert(err, NotNil)
	gofail.Disable("github.com/daiguadaidai/tidb/store/tikv/tikvStoreSendReqResult")

	gofail.Enable("github.com/daiguadaidai/tidb/store/tikv/tikvStoreSendReqResult", `return("GCNotLeader")`)
	regionErr, err = taskWorker.doGCForRegion(bo, 20, loc.Region)
	c.Assert(regionErr.GetNotLeader(), NotNil)
	c.Assert(err, IsNil)
	gofail.Disable("github.com/daiguadaidai/tidb/store/tikv/tikvStoreSendReqResult")

	gofail.Enable("github.com/daiguadaidai/tidb/store/tikv/tikvStoreSendReqResult", `return("GCServerIsBusy")`)
	regionErr, err = taskWorker.doGCForRegion(bo, 20, loc.Region)
